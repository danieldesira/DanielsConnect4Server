...
socketServer.on("connection", async (ws, req) => {
  ...
  try {
    ...
    if (url.searchParams.has("playerColor") && url.searchParams.has("gameId")) {
      ...
      newPlayer = new Player(playerId, name, playerPic, ws, gameId, color);
    } else {
      newPlayer = await Player.attemptNewPlayerPairing(
        playerId,
        name,
        playerPic,
        ws
    ...
    if (opponent) {
      ...
      initialDataToSendNewPlayer.oppenentPic = opponent.getPic();
      ...
      const opponentPic = newPlayer.getPic();
      opponent.getWs().send(
        JSON.stringify({
          opponentName,
          opponentPic,
    ...
    const initialSent = await GameUtils.isInitialSent(
      newPlayer.getGameId(),
      newPlayer.getColor()
    );
    if (!initialSent) {
      ws.send(JSON.stringify(initialDataToSendNewPlayer));
      await GameUtils.updateInitialSent(
        newPlayer.getGameId(),
        newPlayer.getColor()
      );
    }
    ...
    ws.on("message", async (data: string) => {
      ...
      if (opponent) {
        if (
          ...
          GameMessage.isActionMessage(messageData)
        ) {
      ...
      const interval = setInterval(async () => {
        if (!Player.isPlayerConnected(newPlayer)) {
          ...
          if (disconnectCountdown <= 0) {
            ...
            await GameUtils.updateGameFinish(newPlayer.getGameId());
            if (opponent) {
              await GameUtils.updateWinningPlayer(
                opponent.getGameId(),
                opponent.getColor()
              );
            }
