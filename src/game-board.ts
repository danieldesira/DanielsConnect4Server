...
export default class GameBoard {
  ...
  public async load() {
    ...
    try {
      ...
      console.error(
        `Something went wrong for game ${this.gameId} while loading matrix: ${error}`
      );
    } finally {
      await sql.end();
    }
  ...
  public async put(
    ...
    try {
      ...
      console.error(
        `Something went wrong for game ${this.gameId} while putting coin: ${error}`
      );
    } finally {
      await sql.end();
    }
  ...
  private async loadMatrix(sql: Client) {
    const result = await sql.query(
      `SELECT col, row, color FROM Move WHERE game_id = ${this.gameId}`
    );
  ...
  private async getGameDimensions(sql: Client): Promise<BoardDimensions> {
    ...
    const result = await sql.query(
      `SELECT board_dimensions FROM game WHERE id = ${this.gameId}`
    );
    if (result.rowCount) {
