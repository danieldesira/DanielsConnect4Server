...
export default class Player {
    ...
    if (!this.dimensions) {
      ...
      try {
        ...
        const res = await sql.query(
          `SELECT board_dimensions FROM player WHERE id = ${this.id}`
        );
        if (res.rowCount) {
  ...
  public static async attemptNewPlayerPairing(
    id: number,
    name: string,
    pic: string,
    ws: WebSocket
    ...
    if (Player.currentGameId === 0) {
      ...
      try {
        ...
        Player.currentGameId =
          res.rows.length > 0 ? parseInt(res.rows[0].id) : 0;
      } catch (err) {
        console.error(`Failed to fetch current game ID ${err}`);
      } finally {
        await sql.end();
  ...
  public static async getSettings(playerId: number): Promise<PlayerSettings> {
    ...
    try {
      await sql.connect();
      const res = await sql.query(
        `SELECT board_dimensions, theme FROM player WHERE id = ${playerId}`
      );
      if (res.rowCount) {
