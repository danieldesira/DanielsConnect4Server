...
export default class GameUtils {
  ...
  public static async createNewGame(
    gameId: number,
  ...
  public static async updateWinningPlayer(gameId: number, color: Coin) {
    ...
    try {
      ...
      switch (color) {
        case Coin.Red: {
          const res = await sql.query(
            `SELECT player_red FROM game WHERE id = ${gameId}`
          );
        ...
        case Coin.Green: {
          const res = await sql.query(
            `SELECT player_green FROM game WHERE id = ${gameId}`
          );
      ...
      await sql.query(
        `UPDATE game SET winning_player = ${winner} WHERE id = ${gameId}`
      );
    } catch (err) {
      console.error(err);
    } finally {
  ...
  public static async updateDbValue(
    table: string,
    id: number,
    field: string,
    value: string
  ...
  public static async isGamePaired(gameId: number): Promise<boolean> {
    ...
    try {
      await sql.connect();
      const res = await sql.query(
        `SELECT player_red, player_green FROM game WHERE id = ${gameId}`
      );
  ...
  public static async isInitialSent(
    gameId: number,
    playerColor: Coin
  ): Promise<boolean> {
    const sql = new Client(appConfig.connectionString);
    ...
    try {
      ...
      const res = await sql.query(
        `SELECT is_initial_sent_player_${color} FROM game WHERE id = ${gameId}`
      );
      if (res.rowCount) {
  ...
  public static async updateInitialSent(gameId: number, playerColor: Coin) {
    const color = playerColor === Coin.Red ? "red" : "green";
    await this.updateDbValue(
      "game",
      gameId,
      `is_initial_sent_player_${color}`,
      "1::bit"
    );
  }
}
